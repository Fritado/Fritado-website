...
export const saveBlog = async (blogData, status) => {
  const url = `${BLOG_API_ROUTES.SAVE_BLOG}`;
  const formData = new FormData();
  formData.append("blogTopic", blogData.blogTopic);
  formData.append("blogCategory", blogData.blogCategory);
  formData.append("keywords", blogData.keywords); // Convert keywords to array
...
export const fetchBlogs = async () => {
  const url = `${BLOG_API_ROUTES.FETCH_BLOG}`;
  try {
...
export const fetchBlogById = async (id) => {
  const url = `${BLOG_API_ROUTES.FETCH_BLOG_BY_ID}/${id}`;
...
export const updateBlog = async (blogId, updatedBlogData) => {
  const url = `${BLOG_API_ROUTES.EDIT_BLOG_BY_ID}/${blogId}`;
  const formData = new FormData();
...
export const deleteBlog = async (blogId) => {
  const url = `${BLOG_API_ROUTES.DELETE_BLOG_BY_ID}/${blogId}`;
  try {
...
export const fetchLatestBlogs = async () => {
  const url = `${BLOG_API_ROUTES.FETCH_LATEST_BLOG }`
  try {
    const response = await axios.get(url);
    return response.data; 
  } catch (error) {
